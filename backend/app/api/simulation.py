...
from ..services.world_story_service import WorldStoryService
...
@simulation_bp.route('/<simulation_id>/world-story', methods=['GET'])
def get_world_story(simulation_id: str):
    """获取 world story 展示页所需的聚合数据"""
    try:
        ...
        logger.error(f"获取 world story 失败: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
        }), 404
